...
<div class="content-wrapper">
  ...
  <div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="layout-demo-wrapper">
      <div class="card invoice-preview-card">
        <div class="card-body">
            ...
            <div class="col-md-6">
              <dl class="row mb-2">
                ...
                <dd class="col-sm-6 d-flex justify-content-md-end">
                  <div class="w-px-150">
                    <input type="text" class="form-control" disabled placeholder="" value="AUTOSYSTEM" id="invoiceId" />
                  </div>
                </dd>
                <dt class="col-sm-6 mb-2 mb-sm-0 text-md-end">
          ...
          <hr class="my-4 mx-n4" />
          <?php $diagnosis = $data['diagnosisData']; ?>
          <?php $looping = $data['loopingCount']; ?>
          <form class="source-item py-sm-3" action="<?= BASEURL; ?>/transaction/storeRecipe/<?= $patient['id'] ?>/<?= $looping ?>/<?= $diagnosis['id'] ?>" method="POST">
            <div id="service-fee">
                    ...
                    <div class="d-flex border rounded position-relative pe-0">
                      <div class="row w-100 m-0 p-3">
                        <div class="mb-3 col-md-5">
                          ...
                          <?php endif; ?>
                          <select id="selectDrug<?= $i ?>" name="selectDrug<?= $i ?>" class="selectpicker w-100" data-style="btn-default" data-live-search="true" required>
              ...
              <button class="btn btn-secondary d-grid w-100 mb-3" type="submit">
                <span class="d-flex align-items-center justify-content-center text-nowrap">
                  <i class="bx bx-action bx-xs me-1"></i>Save recipe</span>
              </button>
            </div>
          </form>
        </div>
   ...
   <script>
     ...
     let items = []; 
     let serviceFee = 50000;// Array untuk menyimpan total setiap item
     for (let i = 0; i < <?= $looping ?>; i++) {
       ...
       function updatePrice() {
         ...
         fetch("<?= BASEURL; ?>/transaction/getDrugPriceById/" + selectedDrugId).then(response => response.json()).then(data => {
           ...
           let totalWithServiceFee = total + serviceFee;

// Perbarui total dalam elemen total
          totalElement.textContent = formatRupiah(totalWithServiceFee.toString(), "Rp. ");
